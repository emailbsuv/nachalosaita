...
uses
  ...
  IdSSL, IdSSLOpenSSL,System.Generics.Collections,MSHTML,JPEG,IniFiles,
  System.Net.URLClient, System.Net.HttpClient, Soap.SOAPHTTPTrans, Vcl.OleCtrls,DateUtils,
  SHDocVw, Vcl.Imaging.pngimage, Vcl.Tabs, Vcl.ComCtrls, Vcl.TabNotBk,Unit5,Unit6,Unit7,Unit8;
...
begin
  ...
  data := TIdMultiPartFormDataStream.Create;
  data.AddFormField('cookies', Cookie);
  data.AddFormField('userid', id.ToString);
   try
    Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/startoptouserdialog.php',data);
   except
...
//          or (((Users[i].opname = '') and (StrToInt(Users[i].dialogscnt) > 0)) and ((Label25.Caption = 'Запрос диалога')))) then
    ...
    //    panel.InsertComponent(labelMsgcount);
        ...
        Form6.FormShow('НОВЫЙ ПОЛЬЗОВАТЕЛЬ');
      end;
    end;
...
begin
  ...
  HTTP.AllowCookies:=true;
  try
    Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/deletesite.php',data);
  except
    //ShowMessage('Отсутствует связь с сервером' + #10#13);
    Exit;
  end;
...
begin
  ...
  HTTP.AllowCookies:=true;
  try
    Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/saveusercomment.php',data);
  except
    ShowMessage('Отсутствует связь с сервером' + #10#13);
    Exit;
  end;
...
// Application.Terminate;
 ...
 //Form4.Label21.Caption := QueueUpdates.ToString;
 ...
 begin
   ...
   begin
     ...
     begin
       ...
       try
        Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/statusoperator.php',data);
       except
        //ShowMessage('Отсутствует связь с сервером' + #10#13);
        Exit;
       end;
 ...
 try
  Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/updateop.php',data);
 except
  //KillTimer(Form4.Handle, idEvent);
  //ShowMessage('Отсутствует связь с сервером' + #10#13);
  Exit;
        ...
        begin
             ...
             for i2 := 0 to Form4.ScrollBox1.ComponentCount -1 do
              ...
              begin
                 ...
                 if Dialogs1.users[i].newmsgcnt <> '' then Form6.FormShow('ЕСТЬ НОВЫЕ СООБЩЕНИЯ');
 ...
 begin
   ...
   begin
      ...
      try
      Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/setstatusoptying.php',data);
      except
      //ShowMessage('Отсутствует связь с сервером' + #10#13);
      Exit;
      end;
...
//      '1 час 2 мин.','12','3','Здравствуйте, чем я могу вам помочь?','<div class="style7" id="chatform"><div id="mainform" style="transform: translateY(0px);" height="100%"></div></div>');
  ...
  begin
      ...
      try
      Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/getstatususrtying.php',data);
      except
      Exit;
      end;
...
begin
 {/ Form4.InsertUserToDialogs(1,'7','Дмитрий','+380988988692','dima@mail.ru',
  ...
  if (Cookie <>'0') and (logedin = 'true') then
  begin
...
begin
    ...
    begin
      ...
      while Pos((#39),t1)>0 do t1 := StringReplace(t1, #39, '`',[rfReplaceAll, rfIgnoreCase]);
       V := v + t1 + '<br>';DateTimeToString(formattedDateTime,'yyyy-mm-dd hh:nn:ss',myDate);
    end;
    Memo1.Lines.Text := '';
  ...
  data.AddFormField('msg', UTF8Encode(v), 'utf-8').ContentTransfer := '8bit';
   ...
   try
    Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/optousermsg.php',data);
   except
    ShowMessage('Отсутствует связь с сервером' + #10#13);
    Exit;
   end;
...
begin
  ...
  HTTP.AllowCookies:=true;
  try
    Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/blockuser.php',data);
  except
    ShowMessage('Отсутствует связь с сервером' + #10#13);
    Exit;
  end;
...
begin
  ...
  HTTP.AllowCookies:=true;
  try
    Buffer := HTTP.Post('https://nachalosaita.itgroup.kz/sitesusers/closedialog.php',data);
  except
    ShowMessage('Отсутствует связь с сервером' + #10#13);
    Exit;
  end;
...
begin
  ...
  for ArrayElement in JsonArray do begin
      ...
      //for ArrayElement2 in JsonArray2 do begin
        ...
        JsonArray2 := ArrayElement.GetValue<TJSONArray>('department');
...
begin
  ...
  begin
    ...
    if newmsgcnt <> '' then Form6.FormShow('ЕСТЬ НОВЫЕ СООБЩЕНИЯ');
  end
  else
  begin
    ...
    begin
      ...
      WebBrowser1.OleObject.Document.ParentWindow.ScrollBy(0, 100000);
      Form6.FormShow('ЕСТЬ НОВЫЕ СООБЩЕНИЯ');
      end;
    end;
  end;
